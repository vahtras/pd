...
class QuadrupoleList( PointDipoleList ):
    ...
    def total_quadrupole_moment(self):
        """Returns sum of quadrupoles of particles in the list"""
        return sum([p.quadrupole_moment() for p in self])
...
class Quadrupole( PointDipole ):
    ...
    Inherits the PointDipole class with added feature of permanent quadrupole moment
...
#Initialization of
        ...
        if "quadrupole" in kwargs:
            ...
            for ij, (i, j) in enumerate(ut.upper_triangular(2)):
                ...
                self._Q0[i, j] = aij
                self._Q0[j, i] = aij
            #self._Q0 = float( kwargs.get( 'quadrupole' , QUADRUPOLE_ZERO ) )
    ...
    def set_local_field(self, args):
        Quadrupole._assert_vector_like(args)
