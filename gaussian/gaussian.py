...
class GaussianDipoleList( PointDipoleList ):
    """A list class of ``GaussianDipole`` objects"""
    ...
    def __init__(self, pf=None):
        ...
        if pf is not None:
            ...
            for i, line in enumerate(pf):
                ...
                self.append(GaussianDipole(**line_dict))

    @staticmethod
    def from_string(potential):
        """Used to build the ``GaussianDipoleList`` object when the
           potential file is given as a triple quoted string with newlines
        """
        return GaussianDipoleList(iter(potential.split("\n")))
        
    def __str__(self):
        """String representation of class - delegated to list members"""
        return "\n\n".join([str(p) for p in self])

    def append(self, arg):
        """Overriding superclass list append: check if arg is GaussianDipole"""
        if not isinstance(arg,  GaussianDipole):
            print "GaussianDipoleList.append called with object of type", type(arg)
            raise TypeError
        super(GaussianDipoleList, self).append(arg)
    ...
    def dipole_coupling_tensor(self):
        """return the dipole coupling, tensor, describing the
    ...
    def solve_scf_for_external(self, E, max_it=100, threshold=1e-8):
        ...
        for i in range(max_it):
            E_at_point =  self.evaluate_field_at_atoms(external=E)
            #print i, E_at_point
            for p, Ep in zip(self, E_at_point):
                p.set_local_field(Ep)
            residual = norm(E_p0 - E_at_point)
    ...
    def evaluate_field_at_atoms(self, external=None):
        ...
        if external is not None:
            E_at_point = [external + p for p in E_at_point]

        return array(E_at_point)
...
class GaussianDipole( PointDipole ):
    """ 

    Inherits the PointDipole class with added feature of treating the charges as
    ...
    def __init__(self, *args, **kwargs):
        ...
        fixed quantities: 
           ...
           New for GaussianDipole:
           _qdev: charge standard deviation
...
#Initialization of
        ...
        self._q = float( kwargs.get( 'charge' , 0.0 ) )
        self._qdev = float( kwargs.get( 'charge_std' , 0.5 ) )
        self._p0 = kwargs.get( 'dipole' , ZERO_VECTOR )
    ...
    def set_local_field(self, args):
        GaussianDipole._assert_vector_like(args)
...
if __name__ == "__main__":
    ...
    pdl = GaussianDipoleList(open(args.potfile))
